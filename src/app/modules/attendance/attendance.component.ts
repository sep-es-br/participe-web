...
import {faEdit, faUserPlus, faUserTie, faList, faMicrophone, faMicrophoneAlt} from '@fortawesome/free-solid-svg-icons';
      ...
      this.conferenceService.getConferencesWithPresentialMeetings(date).then((confs) => {
        ...
        if (confs.length > 0) {
          ...
          if (roles.includes('Administrator') || roles.includes('Support')) {
            ...
            this.actions.push({name: 'attendance.authorities', url: 'authority-list', icon: faMicrophoneAlt});
