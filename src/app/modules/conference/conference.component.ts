...
export class ConferenceComponent implements OnInit {
  ...
  async showEdit(conference) {
    ...
    let plan = this.plans.find(p => p.value.id === conference.plan.id);
  ...
  async delete(conference) {
    let comments = await this.conferenceService.comments(conference.id);
    let highlights = await this.conferenceService.highlights(conference.id);
  ...
  uploadFileParticipation(event) {
    this.conference.fileParticipation = { ...event.originalEvent.body };
  }

  uploadFileAuthentication(event) {
    this.conference.fileAuthentication = { ...event.originalEvent.body };
  }
