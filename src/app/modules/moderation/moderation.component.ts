...
export class ModerationComponent implements OnInit, OnDestroy {
  ...
  async loadRegionalizationConference() {
    try {
      if (this.conferenceSelect.id) {
        const data = await this.conferenceSvr.getRegionalization(
      ...
      this.messageService.add({
        ...
        detail: this.translate.instant(
          "moderation.error.fetch.regionalization"
        ),
      });
    }
  }
  ...
  populateOptions() {
    ...
    this.fromOptions = this.moderationSrv.FromOptions.map((from) => ({
      value: from,
      label: this.translate.instant(`moderation.label.${from.toLowerCase()}`),
    }));
  ...
  async prepareScreen() {
    ...
    if (
      (strSavedFilter = sessionStorage.getItem("moderationFilter")) !== null
    ) {
  ...
  async loadLocalitiesOptions() {
    try {
      if (this.conferenceSelect.id != null) {
        const data = await this.moderationSrv.getLocalities(
          this.conferenceSelect.id
        );
        this.microregion = _.map(data.localities, ({ id, name }) => ({
          value: id,
          label: name,
        }));
  ...
  async alterModerator(comment: ModerationComments) {
    ...
    const msg = proposal
      ? "moderation.label.confirm_alter_moderator_proposal"
      : "moderation.label.confirm_alter_moderator_comment";
    await this.confirmationService.confirm({
  ...
  private async _publish(comment: ModerationComments) {
    const proposal = comment.type && comment.type === "proposal";
    const msg = proposal
      ? "moderation.label.msg.publish_proposal"
      : "moderation.label.msg.publish_comment";
    try {
      await this.moderationSrv.update({
        status: "Published",
        id: comment.commentId,
